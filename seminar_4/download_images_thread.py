...
# Она использует многопоточный подход для повышения эффективности работы.
...
def time_for_download(urls):
    start_time = time.time()
    with ThreadPoolExecutor(max_workers=5) as executor:  # Задаем максимальное количество потоков
